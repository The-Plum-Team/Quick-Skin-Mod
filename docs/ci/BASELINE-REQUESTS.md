...
first gap. Complete readiness also requires terminal owners, except the requesting producer's
own final job. The review tail also waits for the capacity-resume sibling, so it cannot
...
The collector waits for its exact triggering producer to finish, checks requested source ID
and attempt, runs the existing full admission and rechecks live `master` and that source's
current attempt before publishing.
...
duplicate/foreign reports and post-dispatch visibility races have separate regressions.
...
records 482 versus 549 bounded GET-equivalent operations (+67, 13.9%), and 24 versus one POST. Both
...
and not included in that producer-tail replay. The helper-only count was 525 GETs; the
actual CLI adds one initial current-head guard for each of the 24 producer requests.

An earlier design also checked every successful producer completion. Executing the same
CLI one second after each saved tail added 403 GETs and 24 recovery starts, for 952 GETs
and 25 collector/recovery jobs total; a three-minute sensitivity gave 982 GETs. Those
