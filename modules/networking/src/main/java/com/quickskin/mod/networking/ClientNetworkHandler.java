...
import com.quickskin.mod.client.api.RemoteAppearanceTarget;
...
public class ClientNetworkHandler {
    ...
    private static void executeDueTransparencyReload(long now) {
        ...
        appearanceTarget().reloadSkinsForTransparencyChange();
    }

    private static volatile RemoteAppearanceTarget remoteAppearanceTarget;
    private static java.util.function.BooleanSupplier settingsScreenOpen;

    /** The composition root wires effects before S2C receivers can run. */
    public static synchronized void configureAppearance(
            RemoteAppearanceTarget target, java.util.function.BooleanSupplier settingsOpen) {
        if (remoteAppearanceTarget != null) {
            throw new IllegalStateException("Network appearance target is already configured");
        }
        settingsScreenOpen = java.util.Objects.requireNonNull(settingsOpen, "settingsOpen");
        remoteAppearanceTarget = java.util.Objects.requireNonNull(target, "target");
    }

    private static RemoteAppearanceTarget appearanceTarget() {
        return java.util.Objects.requireNonNull(remoteAppearanceTarget,
                "Network appearance target has not been configured");
    }

    private static boolean isSettingsScreenOpen() {
        appearanceTarget();
        return settingsScreenOpen.getAsBoolean();
    }
