...
The ``mc<version>`` inventory drives the E2E producer matrix (``--kind matrix``), the mod-base
adapter's ``targets`` hook and the visual-review and feature-coverage consumers. Handoff admission
and retention belong to mod-base; this module only derives target identities from the canonical
release matrix.
"""
