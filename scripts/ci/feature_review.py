#!/usr/bin/env python3
"""Curate and reauthenticate selected shared-source captures under protected feature coverage.
...
def expected_captures(bundle_key: str, selected: Any) -> tuple[dict[str, Any], list[str]]:
    ...
    contract = project_contract(coverage.default_contract(), selected)
    captures = {f"{node}/{scenario.scenario}/{role.role}/{step.id}": step.capture
                for node in nodes for scenario in contract.scenarios for role in scenario.roles
...
def validate_selected_manifest(proof: dict[str, Any], manifest: Any, selected: Any) -> None:
    ...
    expected, _nodes = expected_captures(proof["bundle_key"], selected)
...
def curate(api: publisher.Api, *, repository: Path, source_sha: str, source_run_id: int,
           bundle_key: str, compatibility_impact: Any, output: Path, scratch: Path) -> dict[str, Any] | None:
    """Download only the target and, for paired review, its same-run selected Fabric reference."""
    consumer.authenticate_execution(api, repository, run_id=source_run_id,
                                     tested_sha=source_sha, policy_sha=source_sha)
    inventory = api.artifacts(run_id=source_run_id)
    admissions = [item for item in inventory if item["name"] == SELECTION_ARTIFACT]
    if not admissions:
        return None  # The existing complete-profile curator remains authoritative.
    if len(admissions) != 1:
    ...
    plan = targets.plan_targets(packaged, source_run_id=source_run_id,
                                source_branch="master", source_sha=source_sha)
    ...
    data, _matrix_digest, rows = targets._inventory(coverage.DEFAULT_MATRIX, "pr-anchors")
    ...
    for item in required:
        ...
        row = {**by_name[item["name"]], "scenarios": ",".join(run.scenario for run in selected.runs)}
    ...
    proof = {"schema_version": 7, "bundle_key": bundle_key, "matrix_sha256": target["matrix_sha256"],
        "source_run_id": source_run_id, "source_branch": "master", "source_sha": source_sha,
        "master_source_sha": source_sha, "implementation_sha": source_sha, "matrix_kind": "pr-anchors",
        "review_mode": target["review_mode"], "compatibility_impact": compatibility_impact,
        "scenario_contract_sha256": selected.contract_sha256,
        ...
        "visual_reference": ({"evidence_kind": "packaged-selection", "artifact": _artifact_record(reference),
            "artifact_node": reference_node, "source_sha": source_sha, "source_run_id": source_run_id,
            "selection_sha256": selected.sha256} if reference is not None else None),
        ...
        "feature_selection": {"admission": selected.to_dict(), "coverage": authenticated}}
...
def verify(api: publisher.Api, proof_path: Path, manifest_path: Path, *, repository: Path,
    ...
    if (not isinstance(proof, dict)
            or set(proof) != coverage.PROOF_KEYS | {"bundle_key", "matrix_sha256", "feature_selection"}
            or type(proof.get("schema_version")) is not int or proof["schema_version"] != 7
    ...
    if proof["review_mode"] == "reference-comparison":
        matrix, _digest, rows = targets._inventory(coverage.DEFAULT_MATRIX, "pr-anchors")
        ...
        if len(references) != 1 or proof["visual_reference"] != {
            "evidence_kind": "packaged-selection", "artifact": _artifact_record(references[0]),
            "artifact_node": node, "source_sha": source_sha, "source_run_id": source_run_id,
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="qsm-selected-review-") as temporary:
            ...
            if args.verify_proof is not None:
                ...
                result = {"selected": True, "selection_sha256": selected.sha256}
            else:
                impact, _digest = coverage._read(args.compatibility_impact)
                proof = curate(api, compatibility_impact=impact, output=args.output, **arguments)
                result = {"selected": proof is not None}
                if proof is not None:
                    result.update(review_mode=proof["review_mode"], generation_sha=proof["source_sha"],
